...
@ModuleInformation(moduleName = "Swing Animations", moduleDesc = "Анимация взмаха руки при ударе", moduleCategory = ModuleCategory.RENDER)
public class SwingAnimations extends Module {
    private final ModeSetting mode = new ModeSetting("Мод", "Никакой", "Никакой", "Smooth", "Self", "Self 2", "Down", "Forward", "Touch", "BlockHit", "Pander", "Curt", "HMI");
    private final SliderSetting power = new SliderSetting("Сила",3, 0F, 10F, 1F).setVisible(() -> !mode.getValue().equals("Pander") && !mode.is("BlockHit") && !mode.getValue().equals("Forward") && !mode.getValue().equals("Curt") && !mode.getValue().equals("Никакой") && !mode.getValue().equals("HMI"));
    ...
    // HMI View Model settings
    public final SliderSetting hmiPosX = new SliderSetting("Смещение X", 0.0, -2.0, 2.0, 0.01).setVisible(() -> mode.getValue().equals("HMI"));
    public final SliderSetting hmiPosY = new SliderSetting("Смещение Y", 0.0, -2.0, 2.0, 0.01).setVisible(() -> mode.getValue().equals("HMI"));
    public final SliderSetting hmiPosZ = new SliderSetting("Смещение Z", 0.0, -2.0, 2.0, 0.01).setVisible(() -> mode.getValue().equals("HMI"));
    public final SliderSetting hmiRotX = new SliderSetting("Вращение X", 0.0, -180.0, 180.0, 1.0).setVisible(() -> mode.getValue().equals("HMI"));
    public final SliderSetting hmiRotY = new SliderSetting("Вращение Y", 0.0, -180.0, 180.0, 1.0).setVisible(() -> mode.getValue().equals("HMI"));
    public final SliderSetting hmiRotZ = new SliderSetting("Вращение Z", 0.0, -180.0, 180.0, 1.0).setVisible(() -> mode.getValue().equals("HMI"));
    public final SliderSetting hmiScale = new SliderSetting("Масштаб", 1.0, 0.1, 3.0, 0.05).setVisible(() -> mode.getValue().equals("HMI"));

    private final HMIRenderer hmiRenderer = new HMIRenderer();

    public boolean isHMI() {
        return mode.getValue().equals("HMI");
    }

    public HMIRenderer getHMIRenderer() {
        hmiRenderer.setSwingAnimations(this);
        return hmiRenderer;
    }
