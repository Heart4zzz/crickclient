...
import zov.crickclient.module.settings.ColorSetting;
...
public class FireFly extends Module {
    ...
    public final ColorSetting customColor = new ColorSetting("Свой цвет", 0xFFFFD700);
    public final SliderSetting trailLength = new SliderSetting("Длина шлейфа", 70, 10, 150, 1);
    public final SliderSetting trailAlpha = new SliderSetting("Прозрачность шлейфа", 0.6f, 0.05f, 1.0f, 0.05f);
    public final SliderSetting fireflySize = new SliderSetting("Размер светлячка", 1.0f, 0.25f, 3.0f, 0.05f);
    public final ModeSetting sprite = new ModeSetting("Спрайт", "Glow",
            "Glow", "Firefly", "Star", "Snowflake", "Heart", "Lightning", "Bubble", "Pumpkin", "Glowstar");
    ...
    private void onPlayerTick(EventTick e) {
        ...
        particles.forEach(p -> p.update(speedMult, maxSpeed, playerPos, maxTrailLength));
    ...
    private void onRender3D(MatrixStack stack, Camera camera, float tickDelta) {
        ...
        float configuredTrailAlpha = trailAlpha.getFloatValue();
        float configuredFireflySize = fireflySize.getFloatValue();
        ...
        for (FireFlyEntity particle : particles) {
            ...
            drawQuad(buffer, localMatrix, 0.35f * configuredFireflySize, renderColor, finalAlpha * 0.6f);
            drawQuad(buffer, localMatrix, 0.22f * configuredFireflySize, renderColor, finalAlpha);
            drawQuad(buffer, localMatrix, 0.10f * configuredFireflySize, 0xFFFFFFFF, finalAlpha);
    ...
    private net.minecraft.util.Identifier spriteTexture() {
        String name = switch (sprite.getValue()) {
        ...
        return net.minecraft.util.Identifier.of("mre", "images/" + name + ".png");
    }
}