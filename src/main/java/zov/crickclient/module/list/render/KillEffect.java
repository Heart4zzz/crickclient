...
import zov.crickclient.event.list.EventPopTotem;
...
import zov.crickclient.util.render.effects.MasEffectRenderer;
...
public class KillEffect extends Module {
    ...
    private final ModeSetting effect = new ModeSetting("Эффект", "Lightning",
            ...
            "Death Skull", "Death Spark", "Diamond Scrap", "Netherite Scrap", "Flash", "Flick", "Smash");
    private final ModeSetting totemEffect = new ModeSetting("Эффект тотема", "Revive",
            "Off", "Revive", "Revive Spark", "Shield Wave");

    private final List<AnimatedEffect> activeEffects = new ArrayList<>();
    private final List<MasInstance> activeMasEffects = new ArrayList<>();
    ...
    public KillEffect() {
        WorldRenderEvents.LAST.register(context -> {
            ...
            for (MasInstance fx : activeMasEffects) {
                MasEffectRenderer.render(fx.effect, context.matrixStack(), context.camera(), fx.position,
                        now - fx.startedAt, fx.duration, fx.size, color);
            }
        });
    }
    ...
    private void onTotemPop(EventPopTotem event) {
        if (totemEffect.is("Off") || event.getPlayer() == null) return;
        String name = switch (totemEffect.getValue()) {
            case "Revive Spark" -> "revive_spark";
            case "Shield Wave" -> "shield_wave";
            default -> "revive";
        };
        PlayerEntity player = event.getPlayer();
        activeMasEffects.add(new MasInstance(name,
                new Vec3d(player.getX(), player.getY() + player.getHeight() * 0.5, player.getZ()),
                System.currentTimeMillis(), 900L, totemEffect.is("Shield Wave") ? 2.2f : 1.2f));
    }
    ...
    public void onUpdate(final EventTick ignored) {
        ...
        activeMasEffects.removeIf(fx -> System.currentTimeMillis() - fx.startedAt >= fx.duration);
    }
    ...
    private void playKillEffect(Entity entity) {
        ...
        String masEffect = switch (effect.getValue()) {
        ...
        if (masEffect != null) {
            float size = effect.is("Smash") ? 2.4f : effect.is("Flash") ? 1.8f : 1.25f;
            activeMasEffects.add(new MasInstance(masEffect, new Vec3d(x, centerY, z),
                    System.currentTimeMillis(), 850L, size));
            return;
        }
