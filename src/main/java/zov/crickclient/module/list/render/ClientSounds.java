...
public class ClientSounds extends Module {
    ...
    private final SliderSetting volume = ModuleSettingDefinitions.clientSoundsVolume();
    ...
    public static void play(boolean enabled) {
        ...
        if (!INSTANCE.isEnabled() && INSTANCE != CrickClient.getInstance().getModuleStorage().get(ClientSounds.class)) {
            return;
        }
        ...
        lastSound = PositionedSoundInstance.master(soundToPlay, 1f, INSTANCE.volume.getFloatValue() / 100.0f);
