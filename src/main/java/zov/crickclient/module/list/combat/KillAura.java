...
public class KillAura extends Module {
    ...
    private void onAttack(EventAttack event) {
        ...
        if (rotation.is("SpookyTime")) {
            spookyTimeAura.onExternalAttack();
        }
        if (rotation.is("FunTime") && event.getEntity() == target) {
            funTimeOnAttack();
            ticksToAttack = 10;
        }
    }
    ...
    private void onUpdate(final EventTick ignored) {
        ...
        if (target != null) {
            ...
            if (rotation.is("FunTime") || rotation.is("SpookyTime") || rotation.is("ReallyWorld")) {
                updateFocusYaw();
            }
    ...
    public boolean canAttack() {
        ...
        if (rotation.is("FunTime")) {
            if (raycastCheck.getValue()) {
                float checkYaw = funTimeTicksSinceAttack > 0 ? lastYaw : mc.player.getYaw();
                float checkPitch = funTimeTicksSinceAttack > 0 ? lastPitch : mc.player.getPitch();
                if (!FunTimeAuraUtil.isLookingAt(checkYaw, checkPitch, distance.getValue(), target, true)) {
                    return false;
                }
            }
        } else if (!RaytraceUtil.rayTrace(player.getRotationVector(), distance.getValue(), target.getBoundingBox())
                && raycastCheck.getValue()) {
            return false;
        }
