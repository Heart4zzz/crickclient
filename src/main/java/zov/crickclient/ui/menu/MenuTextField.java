...
public class MenuTextField {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        DrawUtil.drawRoundBlur(x, ay, width, height, RADIUS,
                ColorProvider.rgba(180, 190, 220, (int) (80 * ap)), 8f);

        int border = ColorProvider.interpolateColor(
                MenuColors.withAlpha(MenuColors.BTN_BORDER, (int) (100 * ap)),
                MenuColors.withAlpha(MenuColors.BTN_BORDER_HOVER, (int) (180 * ap)),
                fa);
        DrawUtil.drawRound(x - 0.5f, ay - 0.5f, width + 1f, height + 1f, RADIUS + 0.5f, border);
        DrawUtil.drawRound(x, ay, width, height, RADIUS,
                MenuColors.withAlpha(MenuColors.PANEL_INNER, (int) (220 * ap)));
        ...
        if (text.isEmpty() && !focused) {
            DrawUtil.drawText(Fonts.SFREGULAR.get(), placeholder, taX, textY,
                    MenuColors.withAlpha(MenuColors.ACCENT_MUTED, (int) (160 * ap)), FONT_SIZE);
        } else {
            if (hasSelection()) {
    ...
    private void updateScroll(float areaW) {
        ...
        if (caretPos - scrollX > areaW - 2f) scrollX = caretPos - areaW + 2f;
        if (caretPos - scrollX < 0f) scrollX = caretPos;
        float total = widthOf(text);
        if (total - scrollX < areaW - 2f) scrollX = Math.max(0f, total - areaW + 2f);
    ...
    public void keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        switch (keyCode) {
            ...
            case GLFW.GLFW_KEY_LEFT -> moveCaret(ctrl ? 0 : Math.max(0, caret - 1), shift);
            case GLFW.GLFW_KEY_RIGHT -> moveCaret(ctrl ? text.length() : Math.min(text.length(), caret + 1), shift);
