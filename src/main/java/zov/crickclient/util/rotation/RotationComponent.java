...
public class RotationComponent extends Component {
    ...
    public static void fixMovement(final MoveInputEvent event, final float yaw) {
        ...
        for (float testForward = -1F; testForward <= 1F; testForward++) {
            for (float testStrafe = -1F; testStrafe <= 1F; testStrafe++) {
                ...
                final double testAngle = MathHelper.wrapDegrees(Math.toDegrees(direction(mc.player.getYaw(), testForward, testStrafe)));
    ...
    public void onEvent(MoveInputEvent event) {
        ...
        if (aura.isEnabled() && aura.getTarget() != null && !aura.moveFix.is("Нет")) {
            return;
        }
        if (isRotating()) {
            float yaw = FreeLookComponent.isActive()
