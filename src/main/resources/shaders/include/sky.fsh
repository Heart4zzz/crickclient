...
vec3 energySky(vec3 dir, float up, float aboveH) {
    ...
    // Используем цвета из uniform'ов (зависят от темы)
...
vec3 cosmicVeilSky(vec3 dir, float up, float aboveH) {
    ...
    // --- Deep base from theme ---
    vec3 sky = mix(SkyZenith * 0.5, SkyHorizon * 0.4, pow(up, 0.7));

    // --- Nebula body: uses NebColor1/NebColor2 ---
    ...
    // --- Soft horizontal aurora veil ribbon ---
    ...
    // --- Crescent moon with terminator shading and earthshine ---
    ...
    // Terminator: offset sphere for crescent shape
    ...
    // --- Morning star planet with diffraction spikes ---
...
vec3 deepSpaceSky(vec3 dir, float up, float aboveH) {
    // NebIntensity carries movement speed (0 = static)
    ...
    // Reflection nebula (blue/teal) — wider spread
...
// === MODE 4: Blood Storm — crimson energy field with lightning-like flashes ===
vec3 bloodStormSky(vec3 dir, float up, float aboveH) {
    float t = AnimTime;
    vec3 p = normalize(dir) * 2.6;

    float field = energyField(p + vec3(1.0, -0.5, 3.0), t * 1.3);
    float field2 = energyField(p * 1.4 + vec3(-2.0, 3.0, -1.0), t * 0.9 + 5.0);
    float mist = fbm(p * 1.2 + vec3(t * 0.12, -t * 0.08, t * 0.1));

    vec3 blackRed = SkyZenith * 0.2;
    vec3 crimson  = NebColor1;
    vec3 orange   = NebColor2;
    vec3 pale     = StarColor;

    float cover = 0.45 + 0.55 * smoothstep(-0.6, 0.5, dir.y);
    vec3 sky = mix(blackRed, crimson * 0.3, up * 0.5 + mist * 0.3);
    sky += crimson * field * cover * 0.55;
    sky += orange * pow(field, 2.5) * cover * 0.25;
    sky += pale * pow(field2, 3.0) * cover * 0.15;

    // Lightning flash
    float flash = pow(0.5 + 0.5 * sin(t * 14.0 + p.x * 12.0 - p.z * 8.0), 12.0);
    sky += StarColor * flash * cover * 0.12;

    // Ember particles
    float s1 = starLayer(dir, 80.0, 0.988, 200.0);
    float s2 = starLayer(dir, 45.0, 0.993, 210.0);
    sky += orange * (s1 * 0.6 + s2 * 1.2) * aboveH;

    sky += NebColor1 * 0.3 * pow(1.0 - abs(dir.y), 4.0) * 0.15;
    return sky;
}

// === MODE 5: Ice Aurora — freezing blue/cyan aurora ribbons with bright stars ===
vec3 iceAuroraSky(vec3 dir, float up, float aboveH) {
    float t = AnimTime;

    // Dark icy base from theme
    vec3 darkIce = SkyZenith * 0.15;
    vec3 midBlue = NebColor1 * 0.4;
    vec3 sky = mix(darkIce, midBlue, pow(up, 0.6));

    // Multiple aurora ribbons at different heights
    for (int i = 0; i < 3; i++) {
        float yOff = 0.15 + float(i) * 0.2;
        float ribbonY = dir.y - yOff;
        float ribbon = exp(-ribbonY * ribbonY * (12.0 + float(i) * 6.0));
        float wave = fbm(dir * (2.5 + float(i) * 0.8) + vec3(t * (0.15 + float(i) * 0.05), 0.0, float(i) * 3.0));
        float curtain = 0.5 + 0.5 * sin(dir.x * (8.0 + float(i) * 3.0) + dir.z * 5.0 + t * (0.6 + float(i) * 0.2) + wave * 5.0);

        vec3 auroraColor = mix(NebColor1, NebColor2, curtain);
        auroraColor = mix(auroraColor, StarColor, pow(curtain, 3.0) * 0.4);

        sky += auroraColor * ribbon * curtain * 0.45 * aboveH;
    }

    // Ice crystal sparkles
    float s1 = starLayer(dir, 200.0, 0.955, 300.0);
    float s2 = starLayer(dir, 120.0, 0.970, 310.0);
    float s3 = starLayer(dir, 60.0, 0.985, 320.0);
    vec3 crystalColor = mix(StarColor, vec3(0.95, 0.98, 1.0), hash(floor(vec2(atan(dir.z, dir.x), dir.y) * 30.0)));
    sky += crystalColor * (s1 * 0.4 + s2 * 0.7 + s3 * 1.3) * aboveH;

    sky += NebColor2 * 0.3 * pow(1.0 - abs(dir.y), 5.0) * 0.2;
    return sky;
}

// === MODE 6: Starfall — warm meteor shower sky with shooting stars ===
vec3 starfallSky(vec3 dir, float up, float aboveH) {
    float t = AnimTime;

    // Warm dark base from theme
    vec3 darkAmber = SkyZenith * 0.2;
    vec3 warmBrown = NebColor1 * 0.3;
    vec3 sky = mix(darkAmber, warmBrown, pow(up, 0.5));

    // Warm nebula glow
    float neb = fbm(dir * 2.0 + vec3(5.0, -2.0, 8.0));
    float nebMask = smoothstep(0.35, 0.75, neb) * aboveH;
    sky += NebColor1 * nebMask * 0.5;
    sky += NebColor2 * pow(nebMask, 2.0) * 0.25;

    // Dense warm starfield
    float s1 = starLayer(dir, 180.0, 0.958, 400.0);
    float s2 = starLayer(dir, 100.0, 0.972, 410.0);
    float s3 = starLayer(dir, 50.0, 0.986, 420.0);
    float hv = hash(floor(vec2(atan(dir.z, dir.x), dir.y) * 35.0));
    vec3 starC = mix(StarColor, NebColor2, hv);
    sky += starC * (s1 * 0.35 + s2 * 0.65 + s3 * 1.4) * aboveH;

    // Shooting stars / meteors — multiple trails
    for (int i = 0; i < 5; i++) {
        float seed = float(i) * 7.13;
        float period = 3.0 + hash(vec2(seed, 0.0)) * 4.0;
        float phase = mod(t + seed * 2.0, period) / period; // 0..1
        if (phase > 0.7) continue; // only visible for 70% of cycle

        // Random direction for each meteor
        float mYaw = hash(vec2(seed, 1.0)) * 6.2832;
        float mPitch = 0.2 + hash(vec2(seed, 2.0)) * 0.5;
        vec3 meteorStart = normalize(vec3(cos(mYaw) * cos(mPitch), sin(mPitch), sin(mYaw) * cos(mPitch)));
        vec3 meteorDir = normalize(meteorStart + vec3(-0.3, -0.6, 0.1));

        vec3 meteorPos = meteorStart + meteorDir * phase * 0.8;
        float dist = length(cross(dir - meteorStart, meteorDir));
        float along = dot(dir - meteorStart, meteorDir);
        float trailLen = phase * 0.3;

        float trail = smoothstep(0.008, 0.0, dist) * smoothstep(-0.02, 0.0, along) * smoothstep(trailLen, 0.0, along);
        float brightness = (1.0 - phase) * 2.0;
        sky += vec3(1.0, 0.7, 0.3) * trail * brightness * aboveH;
    }

    sky += vec3(0.2, 0.08, 0.02) * pow(1.0 - abs(dir.y), 4.0) * 0.15;
    return sky;
}

// === MODE 7: Void — deep purple/violet void with pulsing dark energy ===
vec3 voidSky(vec3 dir, float up, float aboveH) {
    float t = AnimTime;
    vec3 p = normalize(dir) * 2.5;

    // Abyss base from theme
    vec3 abyss = SkyZenith * 0.1;
    vec3 deepPurple = NebColor1 * 0.25;
    vec3 sky = mix(abyss, deepPurple, pow(up, 0.4));

    // Void tendrils — warped fbm
...
void main() {
    ...
    if (SkyMode < 0.5) {
        ...
        sky = bloodStormSky(dir, up, aboveH);
    } else if (SkyMode < 5.5) {
        sky = iceAuroraSky(dir, up, aboveH);
    } else if (SkyMode < 6.5) {
        sky = starfallSky(dir, up, aboveH);
    } else {
    ...
    // Below horizon: blend to darker version
    if (dir.y < 0.0) {
